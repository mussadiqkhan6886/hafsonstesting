...
export const generateStaticParams = () => {
  return properties.filter(item => item.type === "buying").map(item => ({
    id: String(item.id)
  }))
}
...
const Sale = async  ({params}: {params: Promise<{id: string}>}) => {
  ...
  return (
    <main className='pt-25'>
       ...
       <section className='flex flex-col md:flex-row'>
        ...
        <div>
          <section className='flex flex-col gap-3'>
            <h2 className={`${instrumentSerif.className} text-5xl `}>House for {data.type === 'buying' ? "Sale" : "Letting"}</h2>
      ...
      <aside>
          <div className='flex flex-col gap-4 min-w-[310px] p-5 shadow'>
